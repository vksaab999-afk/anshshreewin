...
async def send_welcome_content(context: ContextTypes.DEFAULT_TYPE, user_id: int, first_name: str):
    try:
        ...
        # 1. Tutorial Video (Forwarded to keep exact style)
        ...
        # 2. VIP Hack File (Forwarded to keep exact original icons/formatting)
        await context.bot.forward_message(
            chat_id=user_id,
            from_chat_id=SOURCE_CHAT_ID,
        ...
        # 3. Audio Note with Button (Using copy_message to support button attachment)
